...
const { Point, Emitter } = require("lumine");
...
class Table extends History {
  static deserialize(state) {
    const table = new Table(state);
    if (!state.modified) table.initializeAfterSetup();
    return table;
  }
  ...
  constructor(state = {}) {
    ...
    if (this.id == null) {
      this.constructor.lastID = (this.constructor.lastID || 0) + 1;
    ...
    this.refcount = 0;
    this.initializeHistory({ modified: Boolean(modified) });
    this.lastModified = Boolean(modified);
  }
  ...
  async save() {
    ...
    if (this.saveHandler) await this.saveHandler(this);
    this.markSavedRevision();
  ...
  serialize() {
    const out = {
      ...
      rows: this.rows,
      id: this.id,
      deserializer: "table-editor/Table",
      version: 1,
    };
  ...
  addColumnAt(index, column, transaction = true, event = true) {
    if (this.isDestroyed()) {
      throw new Error("Can't add column to a destroyed table");
    }
    if (index < 0) {
      throw new Error(`Can't add column ${column} at index ${index}`);
    }
    ...
    this.emitModifiedStatusChange();
    if (event) {
      this.emitter.emit("did-add-column", { column, index });
    }

    if (transaction) {
      this.transaction({
        undo() {
          this.removeColumnAt(index, false);
        },
        redo() {
  ...
  removeColumn(column, transaction = true, event = true) {
    if (column == null) {
      throw new Error("Can't remove an undefined column");
    }

    return this.removeColumnAt(
  ...
  removeColumnAt(index, transaction = true, event = true) {
    ...
    if (transaction) {
      var values = this.getColumnValues(index);
    }
    ...
    this.emitModifiedStatusChange();
    if (event) {
      this.emitter.emit("did-remove-column", { column, index });
    }
  ...
  columnRangeFrom(range) {
    ...
    if (range.end > this.getColumnCount()) {
      range.end = this.getColumnCount();
    }
  ...
  swapColumns(columnA, columnB, transaction = true) {
    ...
    if (transaction) {
      this.transaction({
        undo() {
          this.swapColumns(columnA, columnB, false);
        },
        redo() {
          this.swapColumns(columnA, columnB, false);
        },
  ...
  addRowAt(index, values = {}, batch = false, transaction = true) {
    if (this.isDestroyed()) {
      throw new Error("Can't add row to a destroyed table");
    }
    if (index < 0) {
      throw new Error(`Can't add row ${values} at index ${index}`);
    }
    ...
    if (!batch) {
      ...
      if (transaction) {
        this.transaction({
          undo() {
            this.removeRowAt(index, false, false);
          },
          redo() {
            this.addRowAt(index, values, false, false);
          },
        });
      }
    }
  ...
  addRowsAt(index, rows, transaction = true) {
    ...
    const createdRows = rows.map((row, i) =>
      this.addRowAt(index + i, row, true),
    );
    ...
    if (transaction) {
      ...
      this.transaction({
        undo() {
          this.removeRowsInRange(range, false);
        },
        redo() {
          this.addRowsAt(index, rows, false);
        },
      });
    }
  ...
  removeRow(row, batch = false, transaction = true) {
    if (row == null) {
      throw new Error("Can't remove an undefined row");
    }
  ...
  removeRowsInRange(range, transaction = true) {
    ...
    if (transaction) {
      var rowsValues = removedRows.map((row) => row.slice());
    }
    ...
    if (transaction) {
      this.transaction({
        undo() {
          this.addRowsAt(range.start, rowsValues, false);
        },
        redo() {
          this.removeRowsInRange(range, false);
        },
  ...
  removeRowsAtIndices(indices, transaction = true) {
    ...
    const rowsValues = transaction ? removedRows.map((row) => row.slice()) : [];
  ...
  rowRangeFrom(range) {
    ...
    if (range.end > this.getRowCount()) {
      range.end = this.getRowCount();
    }
  ...
  getCells() {
    return this.rows.reduce((cells, row) => cells.concat(row), []);
  }
  ...
  setValueAtPosition(position, value, batch = false, transaction = true) {
    ...
    if (
      ...
      throw new Error(
        `Table::setValueAtPosition called without an invalid position ${position}`,
      );
    }
    ...
    if (!batch) {
      ...
      if (transaction) {
        this.transaction({
          undo() {
            this.setValueAtPosition(position, oldValue, batch, false);
          },
          redo() {
            this.setValueAtPosition(position, value, batch, false);
          },
        });
      }
    }
  ...
  setValuesAtPositions(positions, values, transaction = true) {
    ...
    if (transaction) {
      ...
      this.transaction({
        undo() {
          this.setValuesAtPositions(positions, oldValues, false);
        },
        redo() {
          this.setValuesAtPositions(positions, values, false);
        },
      });
    }
  }
  ...
  setValuesInRange(range, values, transaction = true) {
    ...
    if (transaction) {
      ...
      this.transaction({
        undo() {
          this.setValuesInRange(range, oldValues, false);
        },
        redo() {
          this.setValuesInRange(range, values, false);
        },
      });
    }
  }
