...
const { Point, Emitter, CompositeDisposable } = require("lumine");
...
class TableEditor {
  static initClass() {
    ...
    this.delegatesProperties("order", "direction", {
    ...
    this.delegatesMethods(
      ...
      "setValuesInRange",
      "getValueAtScreenPosition",
      "setValueAtScreenPosition",
      "setValuesAtScreenPositions",
      "setValuesInScreenRange",
      "getRow",
      "getRows",
      "addRow",
      "addRowAt",
      ...
      "setRowHeightAt",
      "getLastRowIndex",
      "getRowIndexAtPixelPosition",
      "getScreenRow",
      "getScreenRowCount",
      "getScreenRows",
      "getScreenRowHeightAt",
      "getScreenRowOffsetAt",
      "setScreenRowHeightAt",
      "getMinimumRowHeight",
      "getScreenRowIndexAtPixelPosition",
      "rowRangeFrom",
      "onDidAddRow",
      "onDidRemoveRow",
      "onDidChange",
      "onDidChangeRowHeight",
      "getScreenColumn",
      "getScreenColumns",
      "getScreenColumnCount",
      "getLastColumnIndex",
      "getScreenColumnIndex",
      "getScreenColumnWidth",
      "setScreenColumnOptions",
      "setScreenColumnWidthAt",
      "getScreenColumnWidthAt",
      "getScreenColumnAlignAt",
      "getScreenColumnOffsetAt",
      "setScreenColumnAlignAt",
      "getScreenColumnIndexAtPixelPosition",
      ...
      "swapColumns",
      "getColumns",
      "removeScreenColumnsInRange",
      "onDidAddColumn",
      "onDidRemoveColumn",
      "onDidChangeColumnOption",
      "onDidRenameColumn",
      "onDidChangeLayout",
      "getScreenCellRect",
      "getScreenCellPosition",
  ...
  static deserialize(state) {
    state.displayTable = lumine.deserializers.deserialize(state.displayTable);
  ...
  shouldPromptToSave({ windowCloseRequested: _windowCloseRequested } = {}) {
  ...
  createCursorAndSelection(position, range) {
    ...
    this.cursorSubscriptions.set(
      ...
      selection.onDidDestroy(() => {
        ...
        this.emitter.emit("did-remove-selection", {
          selection,
          tableEditor: this,
        });
  ...
  serialize() {
    return {
      deserializer: "table-editor/TableEditor",
      version: 1,
  ...
  pasteClipboard(_options = {}) {
    const { text: clipboardText, metadata } =
    ...
    this.table.batchTransaction(() => {
      ...
      if (metadata) {
        ...
        if (values) {
          ...
          if (
            lumine.config.get(
              "table-editor.copyPaste.flattenBufferMultiSelectionOnPaste",
            )
          ) {
            selections.forEach((selection) => selection.fill(clipboardText));
          } else if (
            selections.every((selection) => !selection.spanMoreThanOneCell()) &&
            selections.length === metadata.selections.length
          ) {
            selections.map((selection, i) =>
              selection.fill(
                metadata.selections[i % metadata.selections.length].text,
              ),
            );
          } else {
            switch (
              lumine.config.get(
                "table-editor.copyPaste.distributeBufferMultiSelectionOnPaste",
              )
            ) {
              case "vertically":
          ...
          selections.forEach((selection) =>
            selection.fillValues(plainValues()),
          );
        }
      } else {
        selections.forEach((selection) => selection.fillValues(plainValues()));
      }
    });
  }
  ...
  getSelectedRange() {
    return this.getLastSelection().getRange();
  }
  ...
  mergeSelections() {
    ...
    this.selections = selections.reduce((memo, selectionA) => {
      const isContained = this.getSelections().some(
        (selectionB) =>
  ...
  getCursorsInRowOrder() {
    return this.getCursors().sort(
      (a, b) => a.getPosition().row - b.getPosition().row,
    );
  }

  getCursorsInColumnOrder() {
    return this.getCursors().sort(
      (a, b) => a.getPosition().column - b.getPosition().column,
    );
  }
  ...
  getCursorPositions() {
    return this.getCursors().map((cursor) =>
      this.modelPosition(cursor.getPosition()),
    );
  }
  ...
  getCursorValues() {
    return this.getCursors().map((cursor) =>
      this.getValueAtScreenPosition(cursor.getPosition()),
    );
  }
  ...
  addCursorBelowLastSelection() {
    ...
    this.addCursorAtScreenPosition([range.end.row, position.column]);
  }

  addCursorAboveLastSelection() {
    const range = this.getSelectedRange();
    const position = this.getCursorPosition();
    this.addCursorAtScreenPosition([range.start.row - 1, position.column]);
  }

  addCursorLeftToLastSelection() {
    const range = this.getSelectedRange();
    const position = this.getCursorPosition();
    this.addCursorAtScreenPosition([position.row, range.start.column - 1]);
  }

  addCursorRightToLastSelection() {
    const range = this.getSelectedRange();
    const position = this.getCursorPosition();
    this.addCursorAtScreenPosition([position.row, range.end.column]);
  }
  ...
  moveUp(delta = 1) {
    this.moveCursors((cursor) => cursor.moveUp(delta));
  }

  moveDown(delta = 1) {
    this.moveCursors((cursor) => cursor.moveDown(delta));
  }

  moveLeft(delta = 1) {
    this.moveCursors((cursor) => cursor.moveLeft(delta));
  }

  moveRight(delta = 1) {
    this.moveCursors((cursor) => cursor.moveRight(delta));
  }
  ...
  moveToTop() {
    this.moveCursors((cursor) => cursor.moveToTop());
  }

  moveToBottom() {
    this.moveCursors((cursor) => cursor.moveToBottom());
  }

  moveToLeft() {
    this.moveCursors((cursor) => cursor.moveToLeft());
  }

  moveToRight() {
    this.moveCursors((cursor) => cursor.moveToRight());
  }

  pageUp() {
    this.moveCursors((cursor) => cursor.pageUp());
  }

  pageDown() {
    this.moveCursors((cursor) => cursor.pageDown());
  }

  pageLeft() {
    this.moveCursors((cursor) => cursor.pageLeft());
  }

  pageRight() {
    this.moveCursors((cursor) => cursor.pageRight());
  }

  moveRowDown() {
    if (this.order != null) {
      return this.notifyRowMoveWithOrder();
    }
    ...
    this.initiateCursorManipulation(() => {
      this.table.batchTransaction(() =>
        cursors.forEach((cursor) => cursor.moveRowDown()),
      );
    });
  }
  ...
  moveRowUp() {
    ...
    this.initiateCursorManipulation(() => {
      this.table.batchTransaction(() =>
        cursors.forEach((cursor) => cursor.moveRowUp()),
      );
    });
  }
  ...
  initiateCursorManipulation(block) {
    const originalCursorPositions = this.getCursors().map((cursor) =>
      cursor.getPosition().copy(),
    );

    block.call(this);

    const finalCursorPositions = this.getCursors().map((cursor) =>
      cursor.getPosition().copy(),
    );
    ...
    this.table.amendLastTransaction({
      undo: (commit) => {
        commit.undo();
        this.getCursors().forEach((cursor, i) =>
  ...
  notifyRowMoveWithOrder() {
    lumine.notifications.addWarning(
      "Rows cannot be moved while the table has an unapplied sort order.",
    );
  }
}
