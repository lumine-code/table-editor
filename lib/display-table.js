...
class DisplayTable {
  ...
  getScreenColumnOffsetAt(column) {
    return this.screenColumnOffsets[column];
  }
  ...
  computeScreenColumnOffsets() {
    this.screenColumnOffsets = rangeArray(
      0,
      this.screenColumns.length - 1,
    ).reduce(
      (memo, i) => {
    ...
    subs.add(
      screenColumn.onDidChangeOption((event) => {
        ...
        if (event.option === "width") {
          return this.computeScreenColumnOffsets();
        }
      }),
    );
  }
  ...
  computeRowOffsets() {
    this.rowOffsets = rangeArray(0, this.table.getRowCount() - 1).reduce(
      (m, i) => {
