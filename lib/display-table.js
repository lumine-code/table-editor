...
const { Point, Range, Emitter, CompositeDisposable } = require("lumine");
const { AxisMetrics } = require("@lumine-code/canvas-grid");
const { installDelegation } = require("./model-utils");
...
class DisplayTable {
  ...
  static deserialize(state) {
    if (state.table) {
      state.table = lumine.deserializers.deserialize(state.table);
    }
  ...
  subscribeToTable() {
    this.subscriptions.add(
      this.table.onDidAddColumn(({ column, index }) => {
        this.addScreenColumn(index, { name: column });
      }),
    );

    this.subscriptions.add(
      this.table.onDidRemoveColumn(({ column, index }) => {
        this.removeScreenColumn(index, column);
      }),
    );

    this.subscriptions.add(
      this.table.onDidRenameColumn(({ newName, oldName, index }) => {
        this.screenColumns[index].setOption("name", newName);
        this.emitter.emit("did-rename-column", {
          screenColumn: this.screenColumns[index],
          oldName,
          newName,
          index,
        });
    ...
    this.subscriptions.add(
      this.table.onDidSwapColumns(({ columnA, columnB }) => {
    ...
    this.subscriptions.add(
      this.table.onDidAddRow(({ index }) => {
        this.rowHeights.splice(index, 0, undefined);
      }),
    );

    this.subscriptions.add(
      this.table.onDidRemoveRow(({ index }) => {
        this.rowHeights.splice(index, 1);
      }),
    );

    this.subscriptions.add(
      this.table.onDidChange((event) => {
        this.updateScreenRows();
    ...
    this.subscriptions.add(
      this.table.onDidChangeCellValue((event) => {
        let newEvent;
    ...
    this.subscriptions.add(this.table.onDidDestroy(() => this.destroy()));
  }
  ...
  subscribeToConfig() {
    this.observeConfig({
      ...
      "table-editor.tableEditor.rowHeight": (configRowHeight) => {
        this.configRowHeight = configRowHeight;
        if (this.rowHeights != null && this.screenRows != null) {
          this.computeRowOffsets();
        }
      },
      "table-editor.tableEditor.minimumRowHeight": (configMinimumRowHeight) => {
        this.configMinimumRowHeight = configMinimumRowHeight;
        if (this.rowHeights != null && this.screenRows != null) {
          this.computeRowOffsets();
        }
      },
      "table-editor.tableEditor.columnWidth": (configScreenColumnWidth) => {
        this.configScreenColumnWidth = configScreenColumnWidth;
        if (this.screenColumns != null) {
          this.computeScreenColumnOffsets();
        }
      },
      "table-editor.tableEditor.minimumColumnWidth": (
        configMinimumScreenColumnWidth,
      ) => {
  ...
  observeConfig(configs) {
    for (let config in configs) {
      this.subscriptions.add(lumine.config.observe(config, configs[config]));
    }
  }
  ...
  serialize() {
    let out = {
      deserializer: "table-editor/DisplayTable",
      version: 1,
      rowHeights: this.rowHeights,
  ...
  getScreenColumns() {
    return this.screenColumns.slice();
  }

  getScreenColumnCount() {
    return this.screenColumns.length;
  }

  getScreenColumn(index) {
    return this.screenColumns[index];
  }

  getScreenColumnIndex(column) {
    return this.screenColumns.indexOf(column);
  }

  getLastColumnIndex() {
    return this.screenColumns.length - 1;
  }

  getContentWidth() {
    return this.columnMetrics?.totalSize || 0;
  }
  ...
  setScreenColumnAlignAt(index, align) {
    const screenColumn = this.screenColumns[index];
    if (screenColumn) {
      screenColumn.align = align;
    }
    this.emitter.emit("did-change-layout", this);
  }

  getScreenColumnOffsetAt(column) {
    return this.columnMetrics?.offsetAt(column) || 0;
  }

  getScreenColumnIndexAtPixelPosition(position) {
    return this.columnMetrics?.indexAt(position) ?? -1;
  }
  ...
  addColumnAt(index, column, options = {}, transaction = true) {
    ...
    if (transaction) {
      ...
      this.table.amendLastTransaction({
  ...
  removeColumnAt(index, transaction = true) {
    ...
    if (transaction) {
      const columnOptions = { ...screenColumn.options };
  ...
  removeScreenColumnsInRange(range, transaction = true) {
    ...
    rangeArray(range.start, range.end)
      .reverse()
      .forEach((index) => {
        this.removeColumnAt(index, transaction);
      });
  }

  computeScreenColumnOffsets() {
    const sizes = this.screenColumns.map((column) => column.options.width);
    if (!this.columnMetrics) {
      this.columnMetrics = new AxisMetrics({
        count: this.screenColumns.length,
        defaultSize: this.getScreenColumnWidth(),
        sizes,
      });
    } else {
      this.columnMetrics.setDefaultSize(this.getScreenColumnWidth());
      this.columnMetrics.setItems(this.screenColumns.length, sizes);
    }
  }
  ...
  subscribeToScreenColumn(screenColumn) {
    ...
    subs.add(
      screenColumn.onDidChangeName(({ newName }) => {
  ...
  screenRowToModelRow(row) {
    return this.screenToModelRowsMap[row];
  }

  modelRowToScreenRow(row) {
    return this.modelToScreenRowsMap[row];
  }

  getScreenRows() {
    return this.screenRows.slice();
  }

  getScreenRowCount() {
    return this.screenRows.length;
  }
  ...
  getLastRowIndex() {
    return this.screenRows.length - 1;
  }
  ...
  getScreenRowOffsetAt(row) {
    return this.rowMetrics?.offsetAt(row) || 0;
  }

  getContentHeight() {
    return this.rowMetrics?.totalSize || 0;
  }

  getRowHeight() {
    return this.rowHeight != null ? this.rowHeight : this.configRowHeight;
  }
  ...
  setRowHeightAt(index, height) {
    ...
    this.rowMetrics.setSize(this.modelRowToScreenRow(index), height);
  ...
  getScreenRowIndexAtPixelPosition(position) {
    return this.rowMetrics?.indexAt(position) ?? -1;
  }

  getRowIndexAtPixelPosition(position) {
    return this.screenRowToModelRow(
      this.getScreenRowIndexAtPixelPosition(position),
    );
  }
  ...
  addRowAt(index, row, options = {}, transaction = true) {
    ...
    if (options.height != null) {
      this.setRowHeightAt(index, options.height);
    }
  ...
  removeRowsInRange(range, transaction = true) {
    ...
    const rowHeights = rangeArray(range.start, range.end).map(
      (i) => this.rowHeights[i],
    );
  ...
  removeRowsInScreenRange(range, transaction = true) {
    ...
    if (this.order != null) {
      rowIndices = (() => {
        let result = [];
        for (
          i = range.start, { end } = range, asc = range.start <= end;
          asc ? i < end : i > end;
          asc ? i++ : i--
        ) {
          var asc;
    ...
    if (transaction) {
      return this.table.amendLastTransaction({
        undo: (commit) => {
          commit.undo();
          return (() => {
            let result3 = [];
  ...
  computeRowOffsets() {
    const sizes = this.screenToModelRowsMap.map(
      (modelRow) => this.rowHeights[modelRow],
    );
    if (!this.rowMetrics) {
      this.rowMetrics = new AxisMetrics({
        count: this.getScreenRowCount(),
        defaultSize: this.getRowHeight(),
        sizes,
      });
    } else {
      this.rowMetrics.setDefaultSize(this.getRowHeight());
      this.rowMetrics.setItems(this.getScreenRowCount(), sizes);
    }
  }
  ...
  updateScreenRows() {
    ...
    if (this.order != null) {
      if (typeof this.order === "function") {
        const orderArray = rows.map((row, originalIndex) => ({
          originalIndex,
          row,
        }));
        orderArray.sort(
          (left, right) =>
            this.order(left.row, right.row) ||
            left.originalIndex - right.originalIndex,
        );
        for (i = 0; i < orderArray.length; i++) {
          const { originalIndex, row } = orderArray[i];
          this.screenRows[i] = row;
        ...
        const compare = this.compareScreenRows(this.direction);
        orderArray.sort(
          (left, right) =>
            compare(left, right) || left.originalIndex - right.originalIndex,
        );
    ...
    if (this.order != null) {
      ...
      for (
        let { row } = range.start,
          ...
          asc = range.start.row <= end;
        asc ? row < end : row > end;
        asc ? row++ : row--
      ) {
        for (
          let { column } = range.start,
            end1 = range.end.column,
            asc1 = range.start.column <= end1;
          asc1 ? column < end1 : column > end1;
          asc1 ? column++ : column--
        ) {
