...
class TableSelectionElement extends HTMLElement {
  static initClass() {
    return element(this, "table-editor-selection");
  }
  ...
  setModel(selection) {
    ...
    this.subscriptions.add(
      this.selection.onDidChangeRange(() => this.update()),
    );
  ...
  selectionScrollRect() {
    ...
    return {
      ...
      right:
        this.tableEditor.getScreenColumnOffsetAt(range.end.column - 1) +
        this.tableEditor.getScreenColumnWidthAt(range.end.column - 1),
      bottom:
        this.tableEditor.getScreenRowOffsetAt(range.end.row - 1) +
        this.tableEditor.getScreenRowHeightAt(range.end.row - 1),
    };
  }
}
