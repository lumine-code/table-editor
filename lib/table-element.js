...
class TableElement extends HTMLElement {
  ...
  ensureCanvasGrid() {
    ...
    this.grid.element.addEventListener("scroll", scroll);
    this.grid.element.addEventListener("keydown", keydown);
    this.subscriptions.add(
      new Disposable(() => {
        this.grid?.element.removeEventListener("scroll", scroll);
  ...
  handleCanvasPointerDown(hit, event) {
    ...
    if (hit.zone !== "column") return true;
    if (event.detail > 1) return false;
    if (hit.column === this.tableEditor.order) {
      if (this.tableEditor.direction === -1) this.tableEditor.resetSort();
      else this.tableEditor.toggleSortDirection();
    } else this.tableEditor.sortBy(hit.column);
    return false;
  }
