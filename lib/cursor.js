...
module.exports = class Cursor {
  ...
  destroy(destroySelection = true) {
    ...
    if (destroySelection && selection && !selection.isDestroyed()) {
      selection.destroy();
    }
  }
  ...
  getRange() {
    return new Range(this.position, {
      ...
      column: Math.min(
        this.tableEditor.getScreenColumnCount(),
  ...
  moveColumnRight() {
    if (this.position.column === this.tableEditor.getScreenColumnCount() - 1) {
      return;
    }
    const oldPosition = this.position.copy();
  ...
  pageUp() {
    ...
    const newRow =
      this.position.row -
      lumine.config.get("table-editor.tableEditor.pageMoveRowAmount");
  ...
  pageLeft() {
    ...
    const newColumn =
      this.position.column -
      lumine.config.get("table-editor.tableEditor.pageMoveColumnAmount");
  ...
  cursorMoved(oldPosition, resetSelection = true) {
    ...
    if (resetSelection) {
      this.selection.resetRangeOnCursor();
    }
    const eventObject = {
      cursor: this,
