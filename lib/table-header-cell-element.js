...
class TableHeaderCellElement extends HTMLElement {
  static initClass() {
    return element(this, "table-editor-header-cell");
  }
  ...
  createdCallback() {
    ...
    this.subscriptions.add(
      lumine.tooltips.add(this.editButton, {
        title: "Edit column name",
      }),
    );
    this.subscriptions.add(
      lumine.tooltips.add(this.fitButton, {
        title: "Adjust width to content",
      }),
    );
    this.subscriptions.add(
      lumine.tooltips.add(this.sortButton, {
  ...
  setModel({ column, index }) {
    ...
    this.label.textContent =
      column.name != null ? column.name : columnName(index);
